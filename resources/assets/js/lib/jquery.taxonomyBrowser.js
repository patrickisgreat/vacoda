...
(function($, window, document) {
    ...
    $.taxonomyBrowser = function(el, options) {
        ...
        base.init = function() {
            ...
            if (base.options.source == 'json') {
                ...
                base.processJSON(base.options.json);
                ...
                base.$ul.find('li').each(function(i) {
                    ...
                    if ($this.closest('ul').prev('a').data('duplicate') == "duplicate") {
                        parent = $this.closest('ul').prev('a').data('slug');
                    } else {
                        parent = $this.closest('ul').prev('a').sanitize() || null;
                    }

                    //debug -- probabaly could be refactored -- this library doesn't check for existing
                    //identical ids in child nodes
                    let search_obj = {
                        id: id
                    };

                    let search_keys = _.keys(search_obj);
                    let search_results = _.find(taxArray, function(obj) {
                        return _.all(search_keys, function(key) {
                            if (obj[key] == search_obj[key]) {
                                id = id + "-childof-" + parent;
                                $child.attr({
        ...
        base.clickEvents = function() {
            ...
            base.$el.on('click', 'li', function(e) {
                ...
                var $activeTerm, $parent, $children, $depth, $klass, $url;
                /*console.log('from base.clickEvents');
                 console.log(parent);
                ...
                if (children && children.length && !klass) {
                    ...
                    $this.toggleClass('active').promise().done(function(){
                        ...
                        var activeLiCount = $this.parent().find("li.active").length;

                            if(activeLiCount > 1){

                                $this.parents().eq(2).nextAll().remove();
                            }
                    ...
                    window.bannerCategory = this.getAttribute('data-id');

                    window.bannerPrettyCategory = $(this).find('span').text();

                    //obj of parents
                    var selectedParents = $('div.miller-container').find('.active');

                    window.selectedState = {};

                    var count = 0;
                    $.each(selectedParents, function(key, val) {
                        var dataId = val.getAttribute('data-id');
                        window.selectedState[key] = dataId;
                        count++;
                    });


                    window.selectedState[count] = window.bannerCategory;
                    //final selection

                } else {
                    //If li without children is selected.
                    //final category
                    window.bannerCategory = this.getAttribute('data-id');
                    ...
                    //on click we toggle the active class, and when that is finished we can determine what needs to appear in the next column
                    $this.toggleClass('active').promise().done(function(){

                        var activeLiCount = $this.parent().find("li.active").length;
                        //if there is now only one term selected in the column
                        if(activeLiCount == 1){

                            //if the term that has been clicked is NO LONGER "active" we need to determine if the active term in the column has children we need to toggle now
                            if(!$this.hasClass('active')){

                                var $activeLiSelected = $this.parent().find("li.has-children.active")
                                    if($activeLiSelected [0]){

                                        $activeTerm = $activeLiSelected
                                        $parent = $activeLiSelected.attr('data-id')
                                        $children = base.getChildren($parent)
                                        $depth = Number($activeTerm.closest(base.options.columnclass).data('depth')) + 1
                                        $klass = $activeTerm.hasClass('active')
                                        $url = $activeTerm.find('a').attr("href")

                                        //append children of active term
                                        base.appendTaxonomy({
                                            taxonomy: $children,
                                            depth: $depth,
                                            parent: $parent
                ...
                //Display category chain in DOM

                var millCol = $(".miller--column");
                var mcolumnList = [];
                var subcatArray = [];
                //Looping through all miller columns and find each active term(s), if there is more than one they will be joined together with a comma
                millCol.map(function(idx, col) {

                    var mcolumn = col;

                    //make sure we arent counting miller columns without active terms selected
                    if($(col).find('li.active').length != 0){

                        mcolumnList = $(mcolumn).find("li.active");

                            mcolumnList = mcolumnList.map(function () {
                                var sValue = $(this).find(".title").text().trim();
                                return sValue;
                            }).get().join(' | ');
                        //output is going to be active term(s) as a string
                        subcatArray.push(mcolumnList);
                    }

                });
                //join each columns active terms together with > symbol and show it in the banner details section
                var subcatArrayStr = subcatArray.join(' > ');
                $(".banner-subcategories").text(subcatArrayStr);
                e.preventDefault();
            });

            base.loadState = function() {
    ...
    $.taxonomyBrowser.defaultOptions = {
        ...
        start: '',
        /* ID of the starting taxonomy item. Default : null */
